...
function Chat() {
    ...
    const handleFunctionCall = (functionName, args) => {
        ...
        switch (functionName) {
            ...
            case 'save_customer_data':
                console.log('Saving customer data:', args);
                saveDataToBackend(args).then(() => {
                    console.log('Customer data saved successfully');
                });
                break;
            case 'generate_cost_estimation':
                handleCostEstimation(args).then(() => {
                    console.log('Cost estimation generated successfully');
                });
                break;
            case 'check_appointment_availability':
                handleAppointmentBooking(args).then(() => {
    ...
    const saveDataToBackend = async (data) => {
        if (!sessionId) {
            console.log("Save data ignored for no session Id");
            return;
        }
    ...
    const handleCostEstimation = async (args) => {
        ...
        setUserData((prev) => ({ ...prev, ...costEstimation }));

        await saveDataToBackend(userData);
